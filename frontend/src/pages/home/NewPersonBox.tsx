...
const NewPersonBox = () => {
  ...
  const handleDismiss = async function () {
    await userService.updateSetting({ name: SettingCode.DashboardGuideHide, value: "1" });
    closeGuide();
  };
  return (
    <div>
            ...
            <InlineStack align="space-between" blockAlign="center">
              ...
              <InlineStack gap="200" align="center" blockAlign="center">
                ...
                <Button
                  size="slim"
                  variant="tertiary"
                  icon={colGuide ? ChevronUpIcon : ChevronDownIcon}
                  onClick={toggleGuide}
                />
              </InlineStack>
            </InlineStack>
              ...
              >
                <Box>
                  {userGuideSteps.map((item, ind) => (
                    ...
                    >
                      <div
                        className={currentStart !== ind ? "cursor-pointer" : ""}
                        onClick={() => {
                          ...
                          <Box>
                            <div className="Polaris-CalloutCard">
                              <div className="Polaris-CalloutCard__Content">
                                <div className={currentStart !== ind ? "mt-0.5" : "Polaris-CalloutCard__Title"}>
                                  <h2 className="Polaris-Text--root Polaris-Text--headingSm">{item.title}</h2>
                                </div>
                                <Collapsible
                                ...
                                >
                                  <BlockStack>
                                    <span className="Polaris-Text--root Polaris-Text--bodyMd">
                                      <p dangerouslySetInnerHTML={{ __html: item.dsc }} />
                                    </span>
                                    {item.button !== "" && (
                                      <div className="Polaris-CalloutCard__Buttons">
                                        <Button variant="primary" onClick={() => console.log(999)}>
                                          {item.button}
                                        </Button>
                                      </div>
                                    )}
                              ...
                              {currentStart === ind && (
                                <img alt="" src={item.stepMask} className="Polaris-CalloutCard__Image" />
                              )}
                            </div>
                          </Box>
                        </InlineStack>
