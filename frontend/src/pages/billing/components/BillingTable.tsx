...
import { type ApiResponse, billingService } from "@/api";
...
export default function BillingTable() {
  ...
  const { data, error, isLoading, isFetching } = useQuery<ApiResponse, Error, TableData>({
    ...
    queryFn: async () => {
      try {
        return await billingService.getData(filters);
      } catch (error) {
        console.log(error);
        return {
          code: 0,
  ...
  const rowMarkup = data.list.map(({ id, order_name, charged_at, charge_status, amount }, index) => (
