...
export const rqGetDashboard = (days: string): Promise<DashboardResponse> =>
  request.get(`api/v1/order/dashboard?days=${days}`); // 查询订单统计
...
export const OrderList = (params: OrderListParams): Promise<OrderListResponse> =>
  request.post("api/v1/order/list", params); // 获取订单列表
...
export const GetBillingDetailData = (params: FilterParams): Promise<IResponse> =>
  request.post("api/v1/billing/details", params);
export const GetCurrentPeriod = (): Promise<IResponse> => request.get("api/v1/billing/current");

export const UpdateDashboardGuide = (name: string, open: boolean): Promise<IResponse> =>
  request.post("api/v1/user/step", {
    name: name,
    open: open,
  });
export const UpdateUserSetting = (name: string, val: string): Promise<IResponse> =>
  request.post("api/v1/user/setting", {
    name: name,
    value: val,
  });

export const GetProductTypes = (): Promise<IResponse> => request.get("api/v1/user/product-type");
