...
export const useOrderActions = () => {
  ...
  // 预取特定页面的数据
  const prefetchOrderPage = useCallback(
    async (params: any) => {
  ...
  const updateOrderOptimistically = useCallback(
    (orderId: string, updates: Partial<any>) => {
      queryClient.setQueriesData({ queryKey: orderQueryKeys.lists() }, (oldData: any) => {
        if (!oldData) return oldData;

        return {
          ...oldData,
          orders: oldData.orders.map((order: any) => (order.id === orderId ? { ...order, ...updates } : order)),
        };
      });
    },
    [queryClient]
