...
func (z *zapLogWriter) parseFields(ctx context.Context, args []interface{}) []zap.Field {
	...
	if reqID := ctx.Value(ctxkeys.XRequestID); reqID != nil {
		...
		fields = append(fields, zap.String(ctxkeys.XRequestID.String(), utils.Uuid()))
	}

	// request ip 地址存在就记录
	if ip := ctx.Value(ctxkeys.ReqClientIP); ip != nil {
		reqIP, _ := ip.(string)
		fields = append(fields, zap.String(ctxkeys.ReqClientIP.String(), reqIP))
	}
	...
	// 请求的user-agent
	if reqUa := ctx.Value(ctxkeys.UserAgent); reqUa != nil {
		ua, _ := reqUa.(string)
		fields = append(fields, zap.String(ctxkeys.UserAgent.String(), ua))
	}
